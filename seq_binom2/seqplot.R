...
seqBFplot <- function(n, bf, linetype=NA, xlab="n", ylab="log(bf)", legend.title="Alt", main="", ylim=NA, log.it=TRUE, forH1=TRUE, fontsize=3.2, plain=FALSE) {
	library(ggplot2)
	...
	if (length(n) > 1) {
		if (is.na(linetype[1])) {
			...
			p1 <- p1 + geom_line(aes(linetype=Alt, group=Alt)) + scale_linetype_discrete(legend.title)
		}
	...
	p1 <- p1 + scale_x_continuous(breaks=x.breaks) + coord_cartesian(ylim=ylim) + theme(legend.position="top")
	...
	# if requested, return the ggplot2 object
	if (plain==TRUE) return(p1)
...
# BF_max <- c()
# for (i in ns) {
# 	BF_min <- c(BF_min, as.vector(proportionBF(i/2, i, p=0.5, rscale="ultrawide")))
# 	BF_max <- c(BF_max, as.vector(proportionBF(0, i, p=0.5)))
# }
# seqBFplot(c(ns, ns), c(BF_min, BF_max), linetype=c(rep("Minimal BF", length(ns)), rep("Maximal BF", length(ns))), legend.title="", log.it=TRUE, ylim=c(0.1, 600))